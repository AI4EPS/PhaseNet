...
def extract_picks(preds, file_names=None, begin_times=None, station_ids=None, dt=0.01, phases=["p", "s"], config=None):
    ...
    for i in range(preds.shape[0]):
        ...
        begin_time = datetime.fromisoformat(begin_time.rstrip("Z"))
...
def save_prob_h5(probs, file_names, output_h5):
    if file_names is None:
        file_names = [f"{i:04d}" for i in range(len(probs))]
    elif type(file_names[0]) is bytes:
        file_names = [f.decode().rstrip(".npz") for f in file_names]
    else:
        file_names = [f.rstrip(".npz") for f in file_names]
    for prob, fname in zip(probs, file_names):
        output_h5.create_dataset(fname, data=prob, dtype="float32")
    return 0


def save_prob(probs, file_names, prob_dir):
    if file_names is None:
        file_names = [f"{i:04d}" for i in range(len(probs))]
    elif type(file_names[0]) is bytes:
        file_names = [f.decode().rstrip(".npz") for f in file_names]
